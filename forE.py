...
"""
Tak veľký, aby nebol 0 a zároveň tak malý, aby neplytvalo autami
a/b * T
an/b*T > 0.5
"""
# cycleTime = 4
...
for intersection in intersections:
  ...
  # gcd = 10
  intersection.schedule.times = [(2 if incStr.cars > 0 else 0, incStr)
                                 for incStr in intersection.incomingStreets]
  # intersection.schedule.times.sort(key=lambda x: x[0])
